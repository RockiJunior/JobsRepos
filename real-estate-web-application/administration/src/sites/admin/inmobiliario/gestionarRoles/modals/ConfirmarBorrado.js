...
const ConfirmarBorrado = ({ rol, closeModalDelete, handleDelete }) => {
    return (
        <Modal keyboard={true} show={true} onHide={closeModalDelete} backdrop="static">
            ...
            <Modal.Body>
                <div className='mb-4'>
                    El rol <b>{rol.name}</b> será eliminado y los usuarios que lo tengan asignado quedarán sin permisos.
                    <br />
                    <br />
                    ¿Está seguro de confirmar esta acción?
                </div>
                <div className='d-flex justify-content-end' style={{ gap: 30 }}>
                    {ButtonClose('Cancelar', closeModalDelete)}
                    {ButtonAccept('Confirmar', handleDelete, rol.id)}
                </div>
            </Modal.Body>
        </Modal>
    )
