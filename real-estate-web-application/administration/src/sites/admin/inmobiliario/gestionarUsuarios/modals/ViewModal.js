...
const ViewModal = ({ user, setShow, roles, branchOffices }) => {
    const nombresSucursales = {}
    branchOffices?.map((office) => nombresSucursales[office.id] = office.branch_office_name)
        ...
        <>
            <Modal keyboard={true} show={true} onHide={handleClose} backdrop="static" >
                ...
                <Modal.Body  className='px-4'>
                        ...
                        !user ? "cargando..." : (
                            <>
                                ...
                                <div className='mt-1'>
                                    ...
                                    user.status === 'expired' ? 'Expirado' : '*Deshabilitado'
                                    }
                                </div>
                            </>
                        )
                    ...
                    <div className="mt-2">
                    {
                        user.roleToUser.length === 0 ? "Este usuario no tiene roles asignados" :
                            user.roleToUser.map((role, index) => (
                                <div key={index}>
                                    Rol <b>{role.role.name}</b> asignado en {nombresSucursales[role.branch_office_id]}
                                </div>
                            ))
                    }
                    <br/>
                ...
                <div className='d-flex justify-content-end'>
                    {ButtonClose('Cerrar', handleClose)}
                </div>                
                </Modal.Body>
            </Modal>
        </>
