...
const InfoDdjjNoActividadComercial = () => {
  ...
  const handleIniciarTramite = () => {
    dispatch(
      tramiteCreate(
        { tipoId: tramites.ddjjNoActividadComercial, userId: user.id },
        navigate
      )
    );
  };
  useEffect(() => {
    user &&
      dispatch(
        checkLastProcedureState(user.id, tramites.ddjjNoActividadComercial)
      );
  }, [user]);
...
export default InfoDdjjNoActividadComercial;
