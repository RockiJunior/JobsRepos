...
const InfoSistemaFidelitas = () => {
  ...
  const handleIniciarTramite = () => {
    dispatch(
      tramiteCreate(
        { tipoId: tramites.solicitudUserSistFidelitas, userId: user.id },
        navigate
      )
    );
  };
  useEffect(() => {
    ...
    user &&
      dispatch(
        checkLastProcedureState(user.id, tramites.solicitudUserSistFidelitas)
      );
  }, [user]);
  ...
  return (
    <div className="container">
      ...
      >
        ...
        >
          Solicitud de Usuario para Sistema Fidelitas
        </h4>
        <div>
          {isRegistred ? (
...
export default InfoSistemaFidelitas;
