...
import BuscadorMatriculado from 'components/tramites/components/infoAccordion/components/BuscadorMatriculado';
...
import { getStatusIconFiscalizacion } from 'utils/getStatusIconFiscalizacion';
  ...
  switch (input.tipo) {
    ...
    case 'select':
      return !isDisabled ? (
        ...
        <span className="form-control">
          {input.opciones.find(
            option => option.value === formData[input.nombre]?.value
          )?.label || '-'}
        </span>
      );
...
const FiscalizacionInputComponent = ({
  input,
  handleChange,
  handleChangeFileMultiple,
  formData,
...
FiscalizacionInputComponent.propTypes = {
...
export default FiscalizacionInputComponent;
