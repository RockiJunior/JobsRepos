...
function getPossibleWords(word: string) {
  const vowels = ['a', 'e', 'i', 'o', 'u'];
  const accentVowels = ['á', 'é', 'í', 'ó', 'ú'];
  ...
  for (let i = 0; i < word.length; i++) {
    ...
    if (index !== -1) {
      ...
      for (let j = 0; j < possibleWords.length; j++) {
        ...
        newWords.push(
          possibleWord.substring(0, i) +
            accentVowels[index] +
            possibleWord.substring(i + 1)
        );
      }
...
prisma.$use(async (params, next) => {
  if (params.action === 'findMany' && params.args) {
    ...
    if (matches?.length) {
      ...
      const value = stringArgs.slice(
        firstIndex,
        stringArgs.indexOf('"', firstIndex)
      );
