...
const ChatThread = ({ thread, index, messages }) => {
  ...
  const lastMessage =
    currentMessages?.content[currentMessages.content.length - 1];
  const time =
    lastMessage &&
    new Date(lastMessage?.created_at).toLocaleDateString('es-ar', {
      weekday: 'long',
      year: 'numeric',
      month: 'long',
      day: 'numeric'
  ...
  const day =
    time &&
    time.split(',')[0][0].toUpperCase() +
      time.split(',')[0].split('').slice(1).join('');
